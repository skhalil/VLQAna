...
void OS2LAna::beginJob() {
  ...
  for (int i=0; i<3; i++){
     ...
     h1_[("ht"+suffix[i]).c_str()]   =  bookDir[i]->make<TH1D>( ("ht"+suffix[i]).c_str(), ";H_{T} (AK4 jets) [GeV]", 200, 0., 2000.) ;
     h1_[("st"+suffix[i]).c_str()]   =  bookDir[i]->make<TH1D>( ("st"+suffix[i]).c_str() ,";S_{T} [GeV]", 200, 0., 2000.) ;
