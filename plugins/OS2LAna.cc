...
class OS2LAna : public edm::EDFilter {
  ...
  private:
    ...
    std::string lep                              ;
    edm::ParameterSet genPartInfoPars_           ; 
};
...
OS2LAna::OS2LAna(const edm::ParameterSet & iConfig):
  ...
  bosMass_                (iConfig.getParameter<double>            ("bosMass")),
  ...
  genPartInfoPars_        (iConfig.getParameter<edm::ParameterSet> ("genPartInfoParams"))
{
...
void OS2LAna::beginJob() {
  ...
  for (int i=0; i<3; i++){
     ...
     h1_[("ht"+suffix[i]).c_str()]   =  bookDir[i]->make<TH1D>( ("ht"+suffix[i]).c_str(), ";H_{T} (AK4 jets) [GeV]", 200, 0., 4000.) ;
     h1_[("st"+suffix[i]).c_str()]   =  bookDir[i]->make<TH1D>( ("st"+suffix[i]).c_str() ,";S_{T} [GeV]", 200, 0., 4000.) ;
