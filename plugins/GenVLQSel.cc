...
{ 
   if(!iEvent.isRealData()){
      ...
      else if ( vlqBbZ.size() == 1 && vlqBtW.size() == 1 ) {*bZtW = 1; h1_["bZtW_Evts"]->Fill(1);}
      ...
      if (verbose_){         
         ...
         cout << "bHtW = " << *bHtW <<", bZtW = " << *bZtW <<", bZbH = " << *bZbH << ", tWtW = " << *tWtW <<", bHbH = "<< *bHbH <<", bZbZ = " << *bZbZ << endl;
            //cout << "evt: " << iEvent.id().event() <<",id: " << iEvent.id().luminosityBlock() << ",run: " << iEvent.id().run() << endl;
            //cout << (*bHtW + *bZtW + *bZbH + *tWtW + *bHbH + *bZbZ) << endl;
      }
      //else{
